...
void showMatrixEditor() {
  ...
  sprintf(tmpString, "%" PRIi16 ";%" PRIi16 "=" STD_SPACE_HAIR "%s%s%s", (int16_t)(colVector ? matSelCol+1 : matSelRow+1), (int16_t)(colVector ? 1 : matSelCol+1), aimBuffer[0] == 0 ? STD_SPACE_HAIR : "", (aimBuffer[0] == 0 || aimBuffer[0] == '-') ? "" : " ", nimBufferDisplay);
...
smallFont:
  ...
  if(prefixWidth > 0) X_POS = prefixWidth;
  else if(!forEditor) X_POS = SCREEN_WIDTH - ((colVector ? stringWidth("[]" STD_SUP_T, font, true, true) : stringWidth("[]", font, true, true)) + baseWidth) - (font == &standardFont ? 0 : 1);
...
smallFont:
  ...
  if(tmpMultX) setSystemFlag(FLAG_MULTx);
