...
void generateAllConstants(void) {
  ...
  generateConstantArray("PointToMm",     "+3.527777777777777777777777777777777777777777777777777778e-01");        // cnst078       mm     = pt × 0.0254 / 72 × 1000
  generateConstantArray("InchToMm",      "+2.540000000000000000000000000000000000000000000000000000e+01");        // cnst079       mm     = inch × 0.0254 × 1000
  generateConstantArray("FtToM",         "+3.048000000000000000000000000000000000000000000000000000e-01");        // cnst080       m      = ft × 12 × 0.0254
  generateConstantArray("SfeetToM",      "+3.048006096012192024384048768097536195072390144780289561e-01");        // cnst081       m      = sfeetus × (1200 / 3937)
  generateConstantArray("YardToM",       "+9.144000000000000000000000000000000000000000000000000000e-01");        // cnst082       m      = yard × 3 × 12 × 0.0254
  generateConstantArray("FathomToM",     "+1.828800000000000000000000000000000000000000000000000000e+00");        // cnst083       m      = fathom × 6 × 12 × 0.0254
  generateConstantArray("MiToKm",        "+1.609344000000000000000000000000000000000000000000000000e+00");        // cnst084       km     = mile × 63360 × 0.0254 / 1000
  generateConstantArray("MiSqToKmSq",    "+2.589988110336000000000000000000000000000000000000000000e+00");        // cnst085       km     = mile^2 × (63360 × 0.0254 / 1000)^2
  generateConstantArray("NmiToKm",       "+1.852000000000000000000000000000000000000000000000000000e+00");        // cnst086       km     = nmi × 1852 / 1000
  generateConstantArray("NmiSqToKmSq",   "+3.429904000000000000000000000000000000000000000000000000e+00");        // cnst087       km     = nmi^2 × (1852 / 1000)^2
  generateConstantArray("AuToM",         "+1.495978707000000000000000000000000000000000000000000000e+11");        // cnst088       m      = au × 149597870700
  generateConstantArray("LyToM",         "+9.460730472580800000000000000000000000000000000000000000e+15");        // cnst089       m      = ly × 299792458 × 3600 × 24 × 365.25
  generateConstantArray("PcToM",         "+3.085677581491367278913937957796471610731921160409179801e+16");        // cnst090       m      = pc × 149597870700 × 648000 / pi
  generateConstantArray("LiToM",         "+5.000000000000000000000000000000000000000000000000000000e+02");        // cnst091       m      = lǐ × 500
  generateConstantArray("YinToM",        "+3.000000000000000000000000000000000000000000000000000000e-02");        // cnst092       m      = yǐn / 0.03
  generateConstantArray("CunToM",        "+3.000000000000000000000000000000000000000000000000000000e+01");        // cnst093       m      = cùn / 30
  generateConstantArray("ZhangToM",      "+3.000000000000000000000000000000000000000000000000000000e-01");        // cnst094       m      = zhàng / 0.3
  generateConstantArray("FenToM",        "+3.000000000000000000000000000000000000000000000000000000e+02");        // cnst095       m      = fēn / 300
  generateConstantArray("MiToM",         "+1.609344000000000000000000000000000000000000000000000000e+03");        // cnst096       m      = mile × 63360 × 0.0254
  generateConstantArray("NmiToM",        "+1.852000000000000000000000000000000000000000000000000000e+03");        // cnst097       m      = nmi × 1852
         
  generateConstantArray("Kmphmps",       "+2.777777777777777777777777777777777777777777777777777777e-01");        // cnst098      mps      =  Kmph  * 0.277...   // 2.5/9
  generateConstantArray("RpmDegps",      "+6.000000000000000000000000000000000000000000000000000000e+00");        // cnst099     Degps    =  Rpm * 6  
  generateConstantArray("Mphmps",        "+4.470400000000000000000000000000000000000000000000000000e-01");        // cnst100     mps      =  Mph  * 0.44704     //11.176/25
  generateConstantArray("RpmRadps",      "+1.047197551196597746154214461093167628065723133125035274e-01");        // cnst101     Radps    =  Rpm  * 0.1047   // pi/30
  generateConstantArray("DegRad",        "+1.745329251994329576923690768488612713442871888541725456e-02");        // cnst102     Rad      =  Deg  * pi / 180 
  generateConstantArray("DegGrad",       "+1.111111111111111111111111111111111111111111111111111111e+00");        // cnst103     Grad     =  Deg  * 10 / 9
  generateConstantArray("GradRad",       "+1.570796326794896619231321691639751442098584699687552910e-02");        // cnst104     Rad      =  Grad * pi / 200
       
  generateConstantArray("AccreToHa",     "+4.046856422400000000000000000000000000000000000000000000e-01");        // cnst105      ha     = acre × 0.0254² × 12² × 43560 / 10000
  generateConstantArray("AccreusToHa",   "+4.046872609874252006568529266090790246096621225500515517e-01");        // cnst106      ha     = acreus × (1200 / 3937)² × 43560 / 10000
  generateConstantArray("MuToM2",        "+1.500000000000000000000000000000000000000000000000000000e-03");        // cnst107      m²     = mǔ / 0.0015
  generateConstantArray("FlozukToMl",    "+2.841306250000000000000000000000000000000000000000000000e+01");        // cnst108      ml     = flozuk × 4.54609e-3 / 160 × 1000000
  generateConstantArray("FlozusToMl",    "+2.957352956250000000000000000000000000000000000000000000e+01");        // cnst109      ml     = flozus × 231 × 0.0254³ / 128 × 1000000
  generateConstantArray("GalusToL",      "+3.785411784000000000000000000000000000000000000000000000e+00");        // cnst110      l      = galus × 231 × 0.0254³ × 1000
  generateConstantArray("GalukToL",      "+4.546090000000000000000000000000000000000000000000000000e+00");        // cnst111      l      = galuk × 4.54609e-3 × 1000
  generateConstantArray("QuartToL",      "+1.136522500000000000000000000000000000000000000000000000e+00");        // cnst112      l      = quart × 4.54609e-3 / 4 × 1000
  generateConstantArray("BarrelToM3",    "+1.589872949280000000000000000000000000000000000000000000e-01");        // cnst113      m³     = barrel × 42 × 231 × 0.0254³
  ...
  generateConstantArray("CalToJ",        "+4.186800000000000000000000000000000000000000000000000000e+00");        // cnst123      joule  = calorie × 4.1868
  generateConstantArray("BtuToJ",        "+1.055055852620000000000000000000000000000000000000000000e+03");        // cnst124      joule  = Btu × 1055.05585262
  generateConstantArray("WhToJ",         "+3.600000000000000000000000000000000000000000000000000000e+03");        // cnst125      joule  = Wh × 3600
       
  generateConstantArray("LbfToN",        "+4.448221615260500000000000000000000000000000000000000000e+00");        // cnst126      newton = lbf × 9.80665 × 0.45359237
       
  generateConstantArray("TorrToPa",      "+1.333223684210526315789473684210526315789473684210526316e+02");        // cnst127      pascal = torr × 101325 / 760
  #if (MMHG_PA_133_3224 == 1)       
    generateConstantArray("MmhgToPa",    "+1.333224000000000000000000000000000000000000000000000000e+02");        // cnst128      pascal = mm.Hg × 133.3224
    generateConstantArray("InhgToPa",    "+3.386388960000000000000000000000000000000000000000000000e+03");        // cnst129      pascal = in.Hg × 133.3224 × 25.4
  ...
  generateConstantArray("_108",          "-1.080000000000000000000000000000000000000000000000000000e+02");        // cnst140
  generateConstantArray("_4",            "-4.000000000000000000000000000000000000000000000000000000e+00");        // cnst141
  generateConstantArray("_1",            "-1.000000000000000000000000000000000000000000000000000000e+00");        // cnst142
  generateConstantArray("_1oneE",        "-3.678794411714423215955237701614608674458111310317678345e-01");        // cnst143
  generateConstantArray("1e_49",         "+1.000000000000000000000000000000000000000000000000000000e-49");        // cnst144
  generateConstantArray("1e_37",         "+1.000000000000000000000000000000000000000000000000000000e-37");        // cnst145
  generateConstantArray("1e_24",         "+1.000000000000000000000000000000000000000000000000000000e-24");        // cnst146
  generateConstantArray("1e_6",          "+1.000000000000000000000000000000000000000000000000000000e-06");        // cnst147
  generateConstantArray("1e_4",          "+1.000000000000000000000000000000000000000000000000000000e-04");        // cnst148
  generateConstantArray("1on10",         "+1.000000000000000000000000000000000000000000000000000000e-01");        // cnst149
  generateConstantArray("1on4",          "+2.500000000000000000000000000000000000000000000000000000e-01");        // cnst150
  generateConstantArray("1on3",          "+3.333333333333333333333333333333333333333333333333333333e-01");        // cnst151
  generateConstantArray("1on2",          "+5.000000000000000000000000000000000000000000000000000000e-01");        // cnst152
  generateConstantArray("egamma",        "+5.772156649015328606065120900824024310421593359399235988e-01");        // cnst153
  generateConstantArray("ln2",           "+6.931471805599453094172321214581765680755001343602552541e-01");        // cnst154
  generateConstantArray("root2on2",      "+7.071067811865475244008443621048490392848359376884740366e-01");        // cnst155
  generateConstantArray("piOn4",         "+7.853981633974483096156608458198757210492923498437764552e-01");        // cnst156
  generateConstantArray("root3on2",      "+8.660254037844386467637231707529361834714026269051903140e-01");        // cnst157
  generateConstantArray("9on10",         "+9.000000000000000000000000000000000000000000000000000000e-01");        // cnst158
  generateConstantArray("ln2piOn2",      "+9.189385332046727417803297364056176398613974736377834128e-01");        // cnst159
  generateConstantArray("1",             "+1.000000000000000000000000000000000000000000000000000000e+00");        // cnst160
  generateConstantArray("3on2",          "+1.500000000000000000000000000000000000000000000000000000e+00");        // cnst161
  generateConstantArray("piOn2",         "+1.570796326794896619231321691639751442098584699687552910e+00");        // cnst162
  generateConstantArray("9on5",          "+1.800000000000000000000000000000000000000000000000000000e+00");        // cnst163
  generateConstantArray("2",             "+2.000000000000000000000000000000000000000000000000000000e+00");        // cnst164
  generateConstantArray("ln10",          "+2.302585092994045684017991454684364207601101488628772976e+00");        // cnst165
  generateConstantArray("3piOn4",        "+2.356194490192344928846982537459627163147877049531329366e+00");        // cnst166
  generateConstantArray("3",             "+3.000000000000000000000000000000000000000000000000000000e+00");        // cnst167
  generateConstantArray("pi",            "+3.141592653589793238462643383279502884197169399375105821e+00");        // cnst168
       
  generateConstantArray("4",             "+4.000000000000000000000000000000000000000000000000000000e+00");        // cnst169
  generateConstantArray("3piOn2",        "+4.712388980384689857693965074919254326295754099062658731e+00");        // cnst170
  generateConstantArray("5",             "+5.000000000000000000000000000000000000000000000000000000e+00");        // cnst171
  generateConstantArray("6",             "+6.000000000000000000000000000000000000000000000000000000e+00");        // cnst172
  generateConstantArray("2pi",           "+6.283185307179586476925286766559005768394338798750211642e+00");        // cnst173
  ...
  generateConstantArray("9999",          "+9.999000000000000000000000000000000000000000000000000000e+03");        // cnst204
  generateConstantArray("10000",         "+1.000000000000000000000000000000000000000000000000000000e+04");        // cnst205
  generateConstantArray("25200",         "+2.520000000000000000000000000000000000000000000000000000e+04");        // cnst206
  generateConstantArray("42000",         "+4.200000000000000000000000000000000000000000000000000000e+04");        // cnst207
  generateConstantArray("73766",         "+7.376600000000000000000000000000000000000000000000000000e+04");        // cnst208
  generateConstantArray("1e6",           "+1.000000000000000000000000000000000000000000000000000000e+06");        // cnst209
  generateConstantArray("1e8",           "+1.000000000000000000000000000000000000000000000000000000e+08");        // cnst210
  generateConstantArray("1e9",           "+1.000000000000000000000000000000000000000000000000000000e+09");        // cnst211
  generateConstantArray("2p32",          "+4.294967296000000000000000000000000000000000000000000000e+09");        // cnst212
  generateConstantArray("1e32",          "+1.000000000000000000000000000000000000000000000000000000e+32");        // cnst213
  generateConstantArray("1e_10000",      "+1.000000000000000000000000000000000000000000000000000000e-10000");     // cnst214
  generateConstantArray("995on1000",     "+9.950000000000000000000000000000000000000000000000000000e-01");        // cnst215
  generateConstantArray("1e_32",         "+1.000000000000000000000000000000000000000000000000000000e-32");        // cnst216
  generateConstantArray("rt3",           "+1.732050807568877293527446341505872366942805253810380628e+00");        // cnst217 if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
