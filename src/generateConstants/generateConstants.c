...
void generateAllConstants(void) {
  ...
  generateConstantArray("AccreToHa",     "+4.046856422400000000000000000000000000000000000000000000e-01"); // cnst95       ha     = acre × 0.0254² × 12² × 43560 / 10000
  generateConstantArray("AccreusToHa",   "+4.046872609874252006568529266090790246096621225500515517e-01"); // cnst96       ha     = acreus × (1200 / 3937)² × 43560 / 10000
  generateConstantArray("MuToM2",        "+1.500000000000000000000000000000000000000000000000000000e-03"); // cnst97       m²     = mǔ / 0.0015

  generateConstantArray("FlozukToMl",    "+2.841306250000000000000000000000000000000000000000000000e+01"); // cnst98       ml     = flozuk × 4.54609e-3 / 160 × 1000000
  generateConstantArray("FlozusToMl",    "+2.957352956250000000000000000000000000000000000000000000e+01"); // cnst99       ml     = flozus × 231 × 0.0254³ / 128 × 1000000
  generateConstantArray("GalusToL",      "+3.785411784000000000000000000000000000000000000000000000e+00"); // cnst100      l      = galus × 231 × 0.0254³ × 1000
  generateConstantArray("GalukToL",      "+4.546090000000000000000000000000000000000000000000000000e+00"); // cnst101      l      = galuk × 4.54609e-3 × 1000
  generateConstantArray("QuartToL",      "+1.136522500000000000000000000000000000000000000000000000e+00"); // cnst102      l      = quart × 4.54609e-3 / 4 × 1000
  generateConstantArray("BarrelToM3",    "+1.589872949280000000000000000000000000000000000000000000e-01"); // cnst103      m³     = barrel × 42 × 231 × 0.0254³

  generateConstantArray("CaratToG",      "+2.000000000000000000000000000000000000000000000000000000e-01"); // cnst104      g      = carat × 0.0002 × 1000
  generateConstantArray("OzToG",         "+2.834952312500000000000000000000000000000000000000000000e+01"); // cnst105      g      = oz × (0.45359237 / 16) × 1000
  generateConstantArray("TrozToG",       "+3.110347680000000000000000000000000000000000000000000000e+01"); // cnst106      g      = tr.oz × 0.45359237 × 175 / 12 × 1000
  generateConstantArray("LbToKg",        "+4.535923700000000000000000000000000000000000000000000000e-01"); // cnst107      kg     = lb × 0.45359237     (1 lb (pound) = 16 oz)
  generateConstantArray("StoneToKg",     "+6.350293180000000000000000000000000000000000000000000000e+00"); // cnst107      kg     = stone × 14 × 0.45359237
  generateConstantArray("ShortcwtToKg",  "+4.535923700000000000000000000000000000000000000000000000e+01"); // cnst109      kg     = short cwt × 100 × 0.45359237 (short cwt = short hundredweight)
  generateConstantArray("CwtToKg",       "+5.080234544000000000000000000000000000000000000000000000e+01"); // cnst110      kg     = cwt × 112 × 0.45359237       (cwt = long hundredweight)
  generateConstantArray("ShorttonToKg",  "+9.071847400000000000000000000000000000000000000000000000e+02"); // cnst111      kg     = short ton × 2000 × 0.45359237
  generateConstantArray("TonToKg",       "+1.016046908800000000000000000000000000000000000000000000e+03"); // cnst112      kg     = ton × 2240 × 0.45359237

  generateConstantArray("CalToJ",        "+4.186800000000000000000000000000000000000000000000000000e+00"); // cnst113      joule  = calorie × 4.1868
  generateConstantArray("BtuToJ",        "+1.055055852620000000000000000000000000000000000000000000e+03"); // cnst114      joule  = Btu × 1055.05585262
  generateConstantArray("WhToJ",         "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst115      joule  = Wh × 3600

  generateConstantArray("LbfToN",        "+4.448221615260500000000000000000000000000000000000000000e+00"); // cnst116      newton = lbf × 9.80665 × 0.45359237

  generateConstantArray("TorrToPa",      "+1.333223684210526315789473684210526315789473684210526316e+02"); // cnst117      pascal = torr × 101325 / 760
  #if (MMHG_PA_133_3224 == 1)
    generateConstantArray("MmhgToPa",    "+1.333224000000000000000000000000000000000000000000000000e+02"); // cnst118      pascal = mm.Hg × 133.3224
    generateConstantArray("InhgToPa",    "+3.386388960000000000000000000000000000000000000000000000e+03"); // cnst119      pascal = in.Hg × 133.3224 × 25.4
  #else // (MMHG_PA_133_3224 == 0)
    generateConstantArray("MmhgToPa",    "+1.333223874150000000000000000000000000000000000000000000e+02"); // cnst118      pascal = mm.Hg × 13.5951 × 9.80665
    generateConstantArray("InhgToPa",    "+3.386388640341000000000000000000000000000000000000000000e+03"); // cnst119      pascal = in.Hg × 13.5951 × 9.80665 × 2.54
  #endif // (MMHG_PA_133_3224 == 1)
  generateConstantArray("PsiToPa",       "+6.894757293168361336722673445346890693781387562775125550e+03"); // cnst120      pascal = psi × 0.45359237 × 9.80665 / 0.0254²
  generateConstantArray("BarToPa",       "+1.000000000000000000000000000000000000000000000000000000e+05"); // cnst121      pascal = bar  × 100000
  generateConstantArray("AtmToPa",       "+1.013250000000000000000000000000000000000000000000000000e+05"); // cnst122      pascal = atm × 101325

  generateConstantArray("HpmToW",        "+7.354987500000000000000000000000000000000000000000000000e+02"); // cnst123      watt   = HPM × 75 × 9.80665
  generateConstantArray("HpukToW",       "+7.456998715822702200000000000000000000000000000000000000e+02"); // cnst124      watt   = HPUK × 550 × 0.3048 × 9.80665 × 0.45359237
  generateConstantArray("HpeToW",        "+7.460000000000000000000000000000000000000000000000000000e+02"); // cnst125      watt   = HPE × 746

  generateConstantArray("YearToS",       "+3.155695200000000000000000000000000000000000000000000000e+07"); // cnst126      second = year  × (365.2425 × 24 × 3600)

  generateConstantArray("LbfftToNm",     "+1.355817948331400400000000000000000000000000000000000000e+00"); // cnst127      Nm = lbf×ft × 9.80665 × 0.45359237 × 12 × 0.0254

  generateConstantArray("_4",            "-4.000000000000000000000000000000000000000000000000000000e+00"); // cnst128
  generateConstantArray("_1",            "-1.000000000000000000000000000000000000000000000000000000e+00"); // cnst129
  generateConstantArray("1e_49",         "+1.000000000000000000000000000000000000000000000000000000e-49"); // cnst130
  generateConstantArray("1e_37",         "+1.000000000000000000000000000000000000000000000000000000e-37"); // cnst131
  generateConstantArray("1e_24",         "+1.000000000000000000000000000000000000000000000000000000e-24"); // cnst132
  generateConstantArray("1e_6",          "+1.000000000000000000000000000000000000000000000000000000e-06"); // cnst133
  generateConstantArray("1e_4",          "+1.000000000000000000000000000000000000000000000000000000e-04"); // cnst134
  generateConstantArray("1on10",         "+1.000000000000000000000000000000000000000000000000000000e-01"); // cnst135
  generateConstantArray("1on4",          "+2.500000000000000000000000000000000000000000000000000000e-01"); // cnst136
  generateConstantArray("1on3",          "+3.333333333333333333333333333333333333333333333333333333e-01"); // cnst137
  generateConstantArray("1on2",          "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst138
  generateConstantArray("egamma",        "+5.772156649015328606065120900824024310421593359399235988e-01"); // cnst139
  generateConstantArray("ln2",           "+6.931471805599453094172321214581765680755001343602552541e-01"); // cnst140
  generateConstantArray("root2on2",      "+7.071067811865475244008443621048490392848359376884740366e-01"); // cnst141
  generateConstantArray("piOn4",         "+7.853981633974483096156608458198757210492923498437764552e-01"); // cnst142
  generateConstantArray("9on10",         "+9.000000000000000000000000000000000000000000000000000000e-01"); // cnst143
  generateConstantArray("1",             "+1.000000000000000000000000000000000000000000000000000000e+00"); // cnst144
  generateConstantArray("3on2",          "+1.500000000000000000000000000000000000000000000000000000e+00"); // cnst145
  generateConstantArray("piOn2",         "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst146
  generateConstantArray("9on5",          "+1.800000000000000000000000000000000000000000000000000000e+00"); // cnst147
  generateConstantArray("2",             "+2.000000000000000000000000000000000000000000000000000000e+00"); // cnst148
  generateConstantArray("ln10",          "+2.302585092994045684017991454684364207601101488628772976e+00"); // cnst149
  generateConstantArray("3piOn4",        "+2.356194490192344928846982537459627163147877049531329366e+00"); // cnst150
  generateConstantArray("3",             "+3.000000000000000000000000000000000000000000000000000000e+00"); // cnst151
  generateConstantArray("pi",            "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst152

  generateConstantArray("4",             "+4.000000000000000000000000000000000000000000000000000000e+00"); // cnst153
  generateConstantArray("3piOn2",        "+4.712388980384689857693965074919254326295754099062658731e+00"); // cnst154
  generateConstantArray("5",             "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst155
  generateConstantArray("2pi",           "+6.283185307179586476925286766559005768394338798750211642e+00"); // cnst156
  generateConstantArray("8",             "+8.000000000000000000000000000000000000000000000000000000e+00"); // cnst157
  generateConstantArray("9",             "+9.000000000000000000000000000000000000000000000000000000e+00"); // cnst158
  generateConstantArray("10",            "+1.000000000000000000000000000000000000000000000000000000e+01"); // cnst159
  generateConstantArray("12",            "+1.200000000000000000000000000000000000000000000000000000e+01"); // cnst160
  generateConstantArray("16",            "+1.600000000000000000000000000000000000000000000000000000e+01"); // cnst161
  generateConstantArray("20",            "+2.000000000000000000000000000000000000000000000000000000e+01"); // cnst162
  //generateConstantArray("21",            "+2.100000000000000000000000000000000000000000000000000000e+01"); // cnst163     used for Lanczos N=22
  //generateConstantArray("gammaR",        "+2.311891000000000000000000000000000000000000000000000000e+01"); // cnst164     used for Lanczos N=22
  generateConstantArray("25",            "+2.500000000000000000000000000000000000000000000000000000e+01"); // cnst163
  generateConstantArray("29",            "+2.900000000000000000000000000000000000000000000000000000e+01"); // cnst164     used for Lanczos N=30
  generateConstantArray("30",            "+3.000000000000000000000000000000000000000000000000000000e+01"); // cnst165
  generateConstantArray("gammaR",        "+3.143188335932791233062744140625000000000000000000000000e+01"); // cnst166     used for Lanczos N=30
  generateConstantArray("32",            "+3.200000000000000000000000000000000000000000000000000000e+01"); // cnst167
  generateConstantArray("36",            "+3.600000000000000000000000000000000000000000000000000000e+01"); // cnst168
  generateConstantArray("47",            "+4.700000000000000000000000000000000000000000000000000000e+01"); // cnst169
  generateConstantArray("180onPi",       "+5.729577951308232087679815481410517033240547246656432155e+01"); // cnst170
  generateConstantArray("60",            "+6.000000000000000000000000000000000000000000000000000000e+01"); // cnst171
  generateConstantArray("200onPi",       "+6.366197723675813430755350534900574481378385829618257950e+01"); // cnst172
  generateConstantArray("125",           "+1.250000000000000000000000000000000000000000000000000000e+02"); // cnst173
  generateConstantArray("150",           "+1.500000000000000000000000000000000000000000000000000000e+02"); // cnst174
  generateConstantArray("205",           "+2.050000000000000000000000000000000000000000000000000000e+02"); // cnst175
  generateConstantArray("360",           "+3.600000000000000000000000000000000000000000000000000000e+02"); // cnst176
  generateConstantArray("400",           "+4.000000000000000000000000000000000000000000000000000000e+02"); // cnst177
  generateConstantArray("600",           "+6.000000000000000000000000000000000000000000000000000000e+02"); // cnst178
  generateConstantArray("1000",          "+1.000000000000000000000000000000000000000000000000000000e+03"); // cnst179
  generateConstantArray("2100",          "+2.100000000000000000000000000000000000000000000000000000e+03"); // cnst180
  generateConstantArray("2520",          "+2.520000000000000000000000000000000000000000000000000000e+03"); // cnst181
  generateConstantArray("3600",          "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst182
  generateConstantArray("6000",          "+6.000000000000000000000000000000000000000000000000000000e+03"); // cnst183
  generateConstantArray("9999",          "+9.999000000000000000000000000000000000000000000000000000e+03"); // cnst184
  generateConstantArray("10000",         "+1.000000000000000000000000000000000000000000000000000000e+04"); // cnst185
  generateConstantArray("25200",         "+2.520000000000000000000000000000000000000000000000000000e+04"); // cnst186
  generateConstantArray("42000",         "+4.200000000000000000000000000000000000000000000000000000e+04"); // cnst187
  generateConstantArray("73766",         "+7.376600000000000000000000000000000000000000000000000000e+04"); // cnst188
  generateConstantArray("1e6",           "+1.000000000000000000000000000000000000000000000000000000e+06"); // cnst189
  generateConstantArray("1e8",           "+1.000000000000000000000000000000000000000000000000000000e+08"); // cnst190
  generateConstantArray("1e9",           "+1.000000000000000000000000000000000000000000000000000000e+09"); // cnst191
  generateConstantArray("2p32",          "+4.294967296000000000000000000000000000000000000000000000e+09"); // cnst192
  generateConstantArray("1e32",          "+1.000000000000000000000000000000000000000000000000000000e+32"); // cnst193
  generateConstantArray("1e_10000",      "+1.000000000000000000000000000000000000000000000000000000e-10000"); // cnst194
  generateConstantArray("995on1000",     "+9.950000000000000000000000000000000000000000000000000000e-01"); // cnst195
  generateConstantArray("1e_32",         "+1.000000000000000000000000000000000000000000000000000000e-32"); // cnst196  if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
  ...
  // source: https://www.vttoth.com/FILES/lanczos.tgz
  generateConstantArray51("gammaC00",      "+2.506628274631000502415765284811045253006986740036318756703152698088652909808141644264478158114643"); // cnst197
  generateConstantArray51("gammaC01",      "+90795313438554.12585760728412902207185494121818510422990562390199419594778421509725059250660477478"); // cnst198
  generateConstantArray51("gammaC02",      "-966788810976616.3165238613163922965882171247384377771690792930892852741666385482155753752741846215"); // cnst199
  generateConstantArray51("gammaC03",      "+4800584619652903.130520636355077553806266769741879659498830407152738371167934130745600044623388068"); // cnst200
  generateConstantArray51("gammaC04",      "-14765868961297540.27064904092821149556100711736690133148186310232345861145647352272488652687903457"); // cnst201
  generateConstantArray51("gammaC05",      "+31520009430682538.17457937985920649010992386050798974102315423368438398460245896094052856122203775"); // cnst202
  generateConstantArray51("gammaC06",      "-49582365010416661.09326008795178786489253227385559918969410907133030684376335533671631031226780693"); // cnst203
  generateConstantArray51("gammaC07",      "+59568927402677002.04904143939675965126139196724493177865501398609411221897535112902272116361691451"); // cnst204
  generateConstantArray51("gammaC08",      "-55906342816448463.66899438215049258933959227649607458054232171494476786360919120606779433224203661"); // cnst205
  generateConstantArray51("gammaC09",      "+41579705761882718.81836872026846865595091094440150772658809279820390684724850793073271091274939857"); // cnst206
  generateConstantArray51("gammaC10",      "-24720590950576683.15802797249415999638807846332716128189191944706094647468094435934791589404852933"); // cnst207
  generateConstantArray51("gammaC11",      "+11801286985635343.58575847383935274746074763562173560062003882605616928057523864988435228756829000"); // cnst208
  generateConstantArray51("gammaC12",      "-4528196760270806.995369621666200554735964133084677928395568191894066774034949226537269118163329489"); // cnst209
  generateConstantArray51("gammaC13",      "+1393637064384668.917157722908202033726675908093527246704252104958408303381715387740313008845141904"); // cnst210
  generateConstantArray51("gammaC14",      "-342325016766888.5479936955467723387327600568400337988293619211103486673308239810777313598627315000"); // cnst211
  generateConstantArray51("gammaC15",      "+66578560073752.67246403368251720854813222816576859988564202476567602534436134110254400104404942690"); // cnst212
  generateConstantArray51("gammaC16",      "-10138544793022.41604607361389789580035078467043905508354626409172360042771928364449573017241522124"); // cnst213
  generateConstantArray51("gammaC17",      "+1190960562789.022438954352946350698116714288406439572882388541923119338089933765849690628214181895"); // cnst214
  generateConstantArray51("gammaC18",      "-105857166683.4123935373722836022279857082446927458625016646619942749346232137590435016079220044575"); // cnst215
  generateConstantArray51("gammaC19",      "+6945291327.796451124796820522079456356395826950647769057580507091796596412483352351183618549540841"); // cnst216
  generateConstantArray51("gammaC20",      "-325841232.8464260929688459624630603325071713695605050282752915466327762939975965122976740632863654"); // cnst217
  generateConstantArray51("gammaC21",      "+10491089.04183867431328778246866673594812558539972730107563825597306012268992151854201423652554125"); // cnst218
  generateConstantArray51("gammaC22",      "-219652.9521075940388150629624539015343302845601375714110391179944415326942002537588845815284375311"); // cnst219
  generateConstantArray51("gammaC23",      "+2782.640500292405732331071582785081107276742114500310058012342169019346915906467717358069437084413"); // cnst220
  generateConstantArray51("gammaC24",      "-19.31769482393126228999283928711817461826517031491190754697778125203682359806685621504868658022287"); // cnst221
  generateConstantArray51("gammaC25",      "+0.063813226038364736490677427053094184825560153837400637303741419820260651820063721448175876091640"); // cnst222
  generateConstantArray51("gammaC26",      "-0.000081213345051388882076245151519040492731624505633194015094047296992491188162952243561414155660"); // cnst223
  generateConstantArray51("gammaC27",      "+0.000000028425884712306176285600873249949436913248124688912080334562388569408577988201187292514310"); // cnst224
  generateConstantArray51("gammaC28",      "-0.000000000001515686855682081535390179935440192138970398946754350022046202956603870401666503058594"); // cnst225
  generateConstantArray51("gammaC29",      "+0.000000000000000003727184110478268994444830355775133199862144652804253659035860755294511160286895"); // cnst226

  // Array angle180
  generateConstantArray51("pi_51",         "+3.141592653589793238462643383279502884197169399375105821e+00"); // cnst227
  generateConstantArray51("200",           "+2.000000000000000000000000000000000000000000000000000000e+02"); // cnst228
  generateConstantArray51("180",           "+1.800000000000000000000000000000000000000000000000000000e+02"); // cnst229

  // Array angle90
  generateConstantArray51("piOn2_51",      "+1.570796326794896619231321691639751442098584699687552910e+00"); // cnst230
  generateConstantArray51("100",           "+1.000000000000000000000000000000000000000000000000000000e+02"); // cnst231
  generateConstantArray51("90",            "+9.000000000000000000000000000000000000000000000000000000e+01"); // cnst232

  // Array angle45
  generateConstantArray51("piOn4_51",      "+7.853981633974483096156608458198757210492923498437764552e-01"); // cnst233
  generateConstantArray51("50",            "+5.000000000000000000000000000000000000000000000000000000e+01"); // cnst234
  generateConstantArray51("45",            "+4.500000000000000000000000000000000000000000000000000000e+01"); // cnst235
  ...
  generateConstantArray34("_4712",       "-4.712000000000000000000000000000000000000000000000000000e+03"); // cnst237
  generateConstantArray34("0",           "+0.000000000000000000000000000000000000000000000000000000e+00"); // cnst238
  generateConstantArray34("1e_32",       "+1.000000000000000000000000000000000000000000000000000000e-32"); // cnst239
  generateConstantArray34("1e_6",        "+1.000000000000000000000000000000000000000000000000000000e-06"); // cnst240
  generateConstantArray34("1e_4",        "+1.000000000000000000000000000000000000000000000000000000e-04"); // cnst241
  generateConstantArray34("1on2",        "+5.000000000000000000000000000000000000000000000000000000e-01"); // cnst242
  generateConstantArray34("1",           "+1.000000000000000000000000000000000000000000000000000000e+00"); // cnst243
  generateConstantArray34("2",           "+2.000000000000000000000000000000000000000000000000000000e+00"); // cnst244
  generateConstantArray34("3",           "+3.000000000000000000000000000000000000000000000000000000e+00"); // cnst245
  generateConstantArray34("4",           "+4.000000000000000000000000000000000000000000000000000000e+00"); // cnst246
  generateConstantArray34("5",           "+5.000000000000000000000000000000000000000000000000000000e+00"); // cnst247
  generateConstantArray34("2pi",         "+6.283185307179586476925286766559005768394338798750211642e+00"); // cnst248
  generateConstantArray34("7",           "+7.000000000000000000000000000000000000000000000000000000e+00"); // cnst249
  generateConstantArray34("9",           "+9.000000000000000000000000000000000000000000000000000000e+00"); // cnst250
  generateConstantArray34("12",          "+1.200000000000000000000000000000000000000000000000000000e+01"); // cnst251
  generateConstantArray34("14",          "+1.400000000000000000000000000000000000000000000000000000e+01"); // cnst252
  generateConstantArray34("24",          "+2.400000000000000000000000000000000000000000000000000000e+01"); // cnst253
  generateConstantArray34("28",          "+2.800000000000000000000000000000000000000000000000000000e+01"); // cnst254
  generateConstantArray34("31",          "+3.100000000000000000000000000000000000000000000000000000e+01"); // cnst255
  generateConstantArray34("38",          "+3.800000000000000000000000000000000000000000000000000000e+01"); // cnst256
  generateConstantArray34("60",          "+6.000000000000000000000000000000000000000000000000000000e+01"); // cnst257
  generateConstantArray34("100",         "+1.000000000000000000000000000000000000000000000000000000e+02"); // cnst258
  generateConstantArray34("153",         "+1.530000000000000000000000000000000000000000000000000000e+02"); // cnst259
  generateConstantArray34("275",         "+2.750000000000000000000000000000000000000000000000000000e+02"); // cnst260
  generateConstantArray34("367",         "+3.670000000000000000000000000000000000000000000000000000e+02"); // cnst261
  generateConstantArray34("400",         "+4.000000000000000000000000000000000000000000000000000000e+02"); // cnst262
  generateConstantArray34("1401",        "+1.401000000000000000000000000000000000000000000000000000e+03"); // cnst263
  generateConstantArray34("1461",        "+1.461000000000000000000000000000000000000000000000000000e+03"); // cnst264
  generateConstantArray34("3600",        "+3.600000000000000000000000000000000000000000000000000000e+03"); // cnst265
  generateConstantArray34("4716",        "+4.716000000000000000000000000000000000000000000000000000e+03"); // cnst266
  generateConstantArray34("4800",        "+4.800000000000000000000000000000000000000000000000000000e+03"); // cnst267
  generateConstantArray34("4900",        "+4.900000000000000000000000000000000000000000000000000000e+03"); // cnst268
  generateConstantArray34("5001",        "+5.001000000000000000000000000000000000000000000000000000e+03"); // cnst269
  generateConstantArray34("32075",       "+3.207500000000000000000000000000000000000000000000000000e+04"); // cnst270
  generateConstantArray34("43200",       "+4.320000000000000000000000000000000000000000000000000000e+04"); // cnst271
  generateConstantArray34("65535",       "+6.553500000000000000000000000000000000000000000000000000e+04"); // cnst272
  generateConstantArray34("86400",       "+8.640000000000000000000000000000000000000000000000000000e+04"); // cnst273
  generateConstantArray34("146097",      "+1.460970000000000000000000000000000000000000000000000000e+05"); // cnst274
  generateConstantArray34("274277",      "+2.742770000000000000000000000000000000000000000000000000e+05"); // cnst275
  generateConstantArray34("1e6",         "+1.000000000000000000000000000000000000000000000000000000e+06"); // cnst276
  generateConstantArray34("1729777",     "+1.729777000000000000000000000000000000000000000000000000e+06"); // cnst277
  generateConstantArray34("2p32",        "+4.294967296000000000000000000000000000000000000000000000e+09"); // cnst278
  generateConstantArray34("maxDate",     "+3.155695348699627200000000000000000000000000000000000000e+18"); // cnst279
  generateConstantArray34("maxTime",     "+3.600000000000000000000000000000000000000000000000000000e+19"); // cnst280  if the number of constants changes: don't forget to update NUMBER_OF_CONSTANTS_* in defines.h
