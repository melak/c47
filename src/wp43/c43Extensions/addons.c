...
void greyOutSoftMenuItem(int16_t x, int16_t y, int16_t yy) {
//printf(">>>> #### x=%d y=%d c1st=%d %d %d %d\n",x,y, yy, menu_A_HOME[x + y*6] , kbd_std[menu_A_HOME[x + y*6]    ].primary, kbd_usr[menu_A_HOME[x + y*6]    ].primary);
  if(jm_HOME_ASN && menu_A_HOME[x + y*6] >= 0  &&  softmenu[softmenuStack[0].softmenuId].menuItem == -MNU_HOME &&
    ...
    int16_t yStroke = SCREEN_HEIGHT - yy*23 - 1;
    for(int16_t xStroke=x*67 + 4 - 2; xStroke<x*67 + 62 + 2; xStroke++) {      //JM mod stroke slash cross out
      for (yStroke = SCREEN_HEIGHT - yy*23 + 2; yStroke > SCREEN_HEIGHT - yy*23 -18 - 2; yStroke--){
