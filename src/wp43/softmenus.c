...
/*                                 <---------------------------------------------------------------------- 6 f shifted functions ------------------------------------------------------------------------->  */
/*                                 <---------------------------------------------------------------------- 6 g shifted functions ------------------------------------------------------------------------->  */
TO_QSPI const int16_t menu_PFN[]         = { ITM_LBL,                       ITM_RTN,                    ITM_END,                  ITM_GTO,               ITM_XEQ,                     -MNU_PFN1,          //JM ADDED LBL RTN ...
                                             ITM_INPUT,                     ITM_END,                    ITM_ERR,                  ITM_TICKS,             ITM_PAUSE,                   -MNU_PFN2,
                                             ITM_PSTO,                      ITM_PRCL,                   ITM_VARMNU,               ITM_MVAR,              ITM_CNST,                    ITM_PUTK                      };

TO_QSPI const int16_t menu_PFN1[]        = { 
                                             ITM_R_CLR,                     ITM_R_COPY,                 ITM_R_SORT,               ITM_R_SWAP,            ITM_LocR,                    -MNU_PFN2,
                                             ITM_NULL,                      ITM_NULL,                   ITM_NULL,                 ITM_NULL,              ITM_NULL,                    ITM_POPLR                     };

TO_QSPI const int16_t menu_PFN2[]        = { ITM_MENU,                      ITM_KEYG,                   ITM_KEYX,                 ITM_CLMENU,            ITM_EXITALL,                 -MNU_PFN1,
...
//This Section is generated by UNIT and Submenu Changes documenb

TO_QSPI const int16_t menu_UnitConv[]        = {  
  -MNU_CONVE, -MNU_CONVP, -MNU_CONVM, -MNU_CONVFP, -MNU_CONVA, -MNU_CONVV, 
  ITM_CtoF, ITM_FtoC, ITM_YEARtoS, ITM_StoYEAR, ITM_NULL, -MNU_CONVX, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, 
  ITM_PRtoDB, ITM_DBtoPR, ITM_NMtoLBFFT, ITM_LBFFTtoNM, ITM_FRtoDB, ITM_DBtoFR, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL};
...
TO_QSPI const int16_t menu_ConvA[]        = {   
  ...
  ITM_NULL, ITM_NULL, ITM_MUtoM2, ITM_M2toMU, ITM_NULL, ITM_NULL, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL}; 
TO_QSPI const int16_t menu_ConvE[]        = {   
  ITM_CALtoJ, ITM_JtoCAL, ITM_BTUtoJ, ITM_JtoBTU, ITM_WHtoJ, ITM_JtoWH, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL}; 
TO_QSPI const int16_t menu_ConvFP[]        = {  
  ITM_LBFtoN, ITM_NtoLBF, ITM_PAtoBAR, ITM_BARtoPA, ITM_PSItoPA, ITM_PAtoPSI, 
  ITM_INCHHGtoPA, ITM_PAtoINCHHG, ITM_TORtoPA, ITM_PAtoTOR, ITM_ATMtoPA, ITM_PAtoATM, 
  ITM_MMHGtoPA, ITM_PAtoMMHG, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL}; 
TO_QSPI const int16_t menu_ConvM[]        = {   
  ...
  ITM_LIANGtoKG, ITM_KGtoLIANG, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_JINtoKG, ITM_KGtoJIN}; 
...
TO_QSPI const int16_t menu_ConvX[]        = {   
  ITM_AUtoM, ITM_MtoAU, ITM_LYtoM, ITM_MtoLY, ITM_PCtoM, ITM_MtoPC, 
  ...
  ITM_LItoM, ITM_MtoLI, ITM_YINtoM, ITM_MtoYIN, ITM_ZHANGtoMb, ITM_MtoZHANGb, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, 
  ITM_CHItoM, ITM_MtoCHI, ITM_CUNtoM, ITM_MtoCUN, ITM_FENtoM, ITM_MtoFEN, 
  ITM_FATHOMtoM, ITM_MtoFATHOM, ITM_POINTtoMM, ITM_MMtoPOINT, ITM_FTUStoM, ITM_MtoFTUS, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, 
  ITM_MItoKM, ITM_KMtoMI, ITM_NMItoKM, ITM_KMtoNMI, ITM_NULL, ITM_NULL}; 
TO_QSPI const int16_t menu_ConvV[]        = {   
  ITM_GLUKtoL, ITM_LtoGLUK, ITM_QTtoL, ITM_LtoQT, ITM_GLUStoL, ITM_LtoGLUS, 
  ITM_FZUKtoML, ITM_MLtoFZUK, ITM_BARRELtoM3, ITM_M3toBARREL, ITM_FZUStoML, ITM_MLtoFZUS, 
  ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL, ITM_NULL}; 
...
/* 107 */  {.menuItem = -MNU_ASN_N,       .numItems = sizeof(menu_ASN_N      )/sizeof(int16_t), .softkeyItem = menu_ASN_N       },  //JM USER NORMAL
/* 108 */  {.menuItem = -MNU_ASN,         .numItems = sizeof(menu_ASN        )/sizeof(int16_t), .softkeyItem = menu_ASN         },  //JM USER
/* 109 */  {.menuItem = -MNU_XEQ,         .numItems = sizeof(menu_XEQ        )/sizeof(int16_t), .softkeyItem = menu_XEQ         },  //JM EXEC
/* 110 */  {.menuItem = -MNU_HOME,        .numItems = sizeof(menu_HOME       )/sizeof(int16_t), .softkeyItem = menu_HOME        },  //JM HOME
/* 111 */  {.menuItem = -MNU_PLOT,        .numItems = sizeof(menu_PLOT       )/sizeof(int16_t), .softkeyItem = menu_PLOT        },  //JM GRAPH
/* 112 */  {.menuItem = -MNU_ALPHA,       .numItems = sizeof(menu_ALPHA      )/sizeof(int16_t), .softkeyItem = menu_ALPHA       },  //JM ALPHA
/* 113 */  {.menuItem = -MNU_BASE,        .numItems = sizeof(menu_BASE       )/sizeof(int16_t), .softkeyItem = menu_BASE        },  //JM BASE
/* 114 */  {.menuItem = -MNU_EE,          .numItems = sizeof(menu_EE         )/sizeof(int16_t), .softkeyItem = menu_EE          },  //JM EE
/* 115 */  {.menuItem = -MNU_XXEQ,        .numItems = sizeof(menu_XXEQ       )/sizeof(int16_t), .softkeyItem = menu_XXEQ        },  //JM TEXTEDIT
/* 116 */  {.menuItem = -MNU_TRG,         .numItems = sizeof(menu_TRG        )/sizeof(int16_t), .softkeyItem = menu_TRG         },
/* 117 */  {.menuItem = -MNU_SETUP,       .numItems = sizeof(menu_SETUP      )/sizeof(int16_t), .softkeyItem = menu_SETUP       },
/* 118 */  {.menuItem = -MNU_REGR,        .numItems = sizeof(menu_REGR       )/sizeof(int16_t), .softkeyItem = menu_REGR        },
/* 119 */  {.menuItem = -MNU_MODEL,       .numItems = sizeof(menu_MODEL      )/sizeof(int16_t), .softkeyItem = menu_MODEL       },
...
#define YY 100
  ...
  float space = ((x2 - x1) - w1 - w2 - w3 - w4) / 7.0f;   //###
  int16_t midpoint = 3.5*space + w1+w2;           //###
  ...
  // s w1 s w2 s. | .s w3 s w4 s
    ...
    compressString = CS;       //JM compressString
    showString(label0, &standardFont, x1 + space + (rightMostSlot ? 0 : 1), y1 + 1, videoMode, false, false);
    compressString = 0;       //JM compressString

    compressString = CS;       //JM compressString
    showString(STD_RIGHT_ARROW, &standardFont, x1 + space+w1+space + (rightMostSlot ? 0 : 1), y1 + 1, videoMode, false, false);
    compressString = 0;       //JM compressString

    compressString = CS;       //JM compressString
    showString(label1, &standardFont, x2 - space-w4 + (rightMostSlot ? 0 : 1), y1 + 1, videoMode, false, false);
    compressString = 0;       //JM compressString

    compressString = CS;       //JM compressString
    showString(STD_LEFT_ARROW, &standardFont, x2 - space-w3-w4-space + (rightMostSlot ? 0 : 1), y1 + 1, videoMode, false, false);
    compressString = 0;       //JM compressString
    ...
    //   Mid line
    if(x1 >= 0) {
      lcd_fill_rect(x1 + midpoint, y1+4, 1, min(y2, SCREEN_HEIGHT - 1) + 1 - y1 - 2*4, (videoMode == vmNormal ? LCD_EMPTY_VALUE : LCD_SET_VALUE));
    }
}
...
else if( softmenu[m].menuItem  == -MNU_CONVE || softmenu[m].menuItem  == -MNU_CONVP || softmenu[m].menuItem  == -MNU_CONVFP || softmenu[m].menuItem  == -MNU_CONVM || softmenu[m].menuItem  == -MNU_CONVX || softmenu[m].menuItem  == -MNU_CONVV || softmenu[m].menuItem  == -MNU_CONVA || softmenu[m].menuItem  == -MNU_CONST) { 
